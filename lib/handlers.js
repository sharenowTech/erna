...
const commands = async (req, res) => {
  ...
  switch (subcmd) {
    case 'schedule':
      ...
      await storage.setSchedule(user, validLocation, datetime, title)
      return res.json({ text: `Successfully scheduled ${title || 'an event'} in ${validLocation} at ${datetime} (UTC).` })
    default:
