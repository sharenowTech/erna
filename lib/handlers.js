...
const triggerFunnel = async (res, userId, command) => {
  ...
  if (env.locations.list.length === 1) {
    return signup(res, env.locations.list[0], userId)
  }

  return res.json(postman.locationsPrompt(isNewbie, command))
}
...
const commands = async (req, res) => {
  ...
  switch (subcmd) {
    case 'notify':
      const [location = env.locations.list[0]] = args
      const msg = await postman.notify(location)
      slack.sendToAll(msg)

      return res.json(postman.succeededNotify)
    case 'schedule':
    ...
    default:
      return triggerFunnel(res, req.body.user_id, req.body.command)
  }
}
...
const actions = async (req, res) => {
  ...
  switch (action) {
    case 'notify.prompt':
      return triggerFunnel(res, userId)
