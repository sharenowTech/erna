...
const commands = async (req, res) => {
  ...
  switch (subcmd) {
    case 'schedule':
      ...
      await storage.setSchedule(validLocation, datetime)
...
const match = async () => {
  ...
  const cities = await scheduler.find(time)

  if (!cities.length) {
    console.info('Matching skipped.')
    return
  }

  console.info('Matchting started.')
  ...
  console.info('Matchting succeeded.')
...
const schedule = async (req, res) => res.json({
  custom: await storage.listSchedule(),
