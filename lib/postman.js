...
class Postman {
  ...
  get invalidScheduleFormat () {
    const text = random([
      `Schedule failed. Please use the format '<location> <yyyy-mm-dd> <hh:mm>'.`
    ])

    return { text }
  }

  get invalidScheduleLocation () {
    const text = random([
      `Schedule failed. Please choose a valid location: ${env.locations.list.join(', ')}.`
    ])
  ...
  get noMatch () {
    return random([
      ...
      'Tell your colleagues about me, maybe it will work out next time ☝️ I\'m so sorry!.'
    ])
  }
  ...
  async confirmation (location) {
    ...
    return { text: isCustom ? `${baseText}\n\nFYI: it's ${title ? `the ${title}` : 'a scheduled onetime event'} in ${location} 🎉.` : baseText }
  }
  ...
  duplicateSchedule (location, datetime, data) {
  ...
  validSchedule (location, datetime, title) {
    const text = `Successfully scheduled ${title || 'an event'} in ${location} at ${datetime}.`
