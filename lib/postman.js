...
class Postman {
  ...
  async confirmation (location) {
    const nextEvent = await scheduler.getNext(location)

    if (!nextEvent) {
      return { text: 'Unfortunately, there are not any lunch dates planned. Stay tuned!' }
    }

    const { date, isCustom, title } = nextEvent
    const baseText = random([
