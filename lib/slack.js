...
class Slack {
  ...
  async fetchBotId () {
    ...
    while (true) {
      const data = await this.web.users.list({ limit, ...(cursor ? { cursor } : {})})
      const { ok, members, response_metadata } = data

      if (!ok || !members) {
        throw new Error('Cannot fetch members.')
      }

      botUser = members.find((x) => x.name === env.botName)
      cursor = response_metadata && response_metadata.next_cursor

      if (!cursor || botUser) break
    }
