...
class Slack {
  ...
  async triggerMatch (match) {
    if (match.length === 0) {
      console.info('Matched failed: anyone has signed up.')
      return
    }

    if (match.length === 1) {
      console.info('Matched failed: just one has signed up.')
    ...
    const [err, { ok, channel }] = await to(this.openConversation(conversationUsers))

    if (err) {
      console.info(`Open conversation failed: ${err}`)
    }

    if (!ok) {
      console.info('Open conversation failed.')
