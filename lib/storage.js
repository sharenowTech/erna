...
class Storage {
  ...
  list (prop, res) {
    res.set(gatekeeper.sign(this[prop]))
    return res.json(this[prop])
  }

  restore (req, res, params) {
    const restoreUrl = params[0]
    const headers = gatekeeper.sign({})

    axios.get(`${restoreUrl}/locations`, { headers })
      .then((result) => {
        const { data } = result

        if (!gatekeeper.verify(result)) {
          throw Error('Request verification failed.')
        }
