...
module.exports = class InMemoryStorage extends Adapter {
  ...
  async match (locations) {
    ...
    const matchesPerLocation = locationIds.map((x) => (
      this.chunk(this.shuffle(this.locations[x]))
    ))
  ...
  async setSchedule (user, location, datetime) {
    ...
    if (this.schedule[location].includes(datetime)) {
      return {}
    }
  ...
  async setSkip (user, datetime) {
    if (this.skips.includes(datetime)) {
      return {}
    }

    this.schedule.push(datetime)
  }
